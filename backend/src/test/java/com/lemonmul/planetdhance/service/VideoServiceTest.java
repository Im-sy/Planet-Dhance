...
////        em.persist(music3);
        ...
        for(int i=0;i<5;i++){
            ...
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user1").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user1").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(32l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(32l).get().getTitle(),TagType.TITLE));
            for(int j=0;j<i;j++){
                video.addHit();
            }
            video.addLikeWeight();
        ...
        for(int i=5;i<10;i++){
            ...
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user1").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user1").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(28l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(28l).get().getTitle(),TagType.TITLE));
            for(int j=4;j<i;j++){
                video.addHit();
            }
            video.addLikeWeight();
...
//        em.persist(Like.createLike(video1,user2));
        for(int i=10;i<15;i++){
            ...
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user2").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(userRepo.findByNickname("user2").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(28l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findByNameAndType(musicRepo.findById(28l).get().getTitle(),TagType.TITLE));
            for(int j=7;j<i;j++){
                video.addHit();
