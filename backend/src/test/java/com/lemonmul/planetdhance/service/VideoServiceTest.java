...
//        printHitLikeList(1, 5,music);
    ...
    private void createArtistAndMusicTable(){
        ...
        em.persist(artist);
        music=Music.createMusic("Sparkling",artist,"/resource/artist/img/sparkling_img.jpg","model url1","/resource/artist/video/sparkling_guide.mp4","https://youtu.be/lDV5cM9YE4g", LocalDateTime.of(2022,6,24,0,0));
        em.persist(music);
        em.persist(Tag.createTag(music.getArtist().getName(),TagType.ARTIST,music.getArtist().getImgUrl()));
        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
        ...
        em.persist(artist);
        music=Music.createMusic("TT",artist,"/resource/artist/img/TT_img.jpg","model url1","/resource/artist/video/TT_guide.mp4","https://youtu.be/ePpPVE-GGJw", LocalDateTime.of(2022,6,24,0,0));
        em.persist(music);
        em.persist(Tag.createTag(music.getArtist().getName(),TagType.ARTIST,music.getArtist().getImgUrl()));
        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
        ...
        em.persist(artist);
        music=Music.createMusic("FOREVER 1",artist,"/resource/artist/img/Forever1_img.jpg","model url1","/resource/artist/video/Forever1_guide.mp4","https://youtu.be/Qpf26PtBXgo", LocalDateTime.of(2022,6,24,0,0));
        em.persist(music);
        em.persist(Tag.createTag(music.getArtist().getName(),TagType.ARTIST,music.getArtist().getImgUrl()));
        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
...
//        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
        ...
        em.persist(artist);
        music=Music.createMusic("HOT",artist,"/resource/artist/img/hot_img.jpg","model url1","/resource/artist/video/hot_guide.mp4","https://youtu.be/gRnuFC4Ualw", LocalDateTime.of(2022,6,24,0,0));
        em.persist(music);
        em.persist(Tag.createTag(music.getArtist().getName(),TagType.ARTIST,music.getArtist().getImgUrl()));
        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
        ...
        em.persist(artist);
        music=Music.createMusic("TOMBOY",artist,"/resource/artist/img/Tomboy_img.jpg","model url1","/resource/artist/video/Tomboy_guide.mp4","https://youtu.be/Jh4QFaPmdss", LocalDateTime.of(2022,6,24,0,0));
        em.persist(music);
        em.persist(Tag.createTag(music.getArtist().getName(),TagType.ARTIST,music.getArtist().getImgUrl()));
        em.persist(Tag.createTag(music.getTitle(),TagType.TITLE, music.getImgUrl()));
    }
...
////        em.persist(music3);
        ...
        for(int i=0;i<5;i++){
            Video video = Video.createVideo("/temp/c2d6f99a-8e0c-4413-a8ab-6e809b6b7d1b.webm", "/temp/9e7de94f-a832-4af3-9125-fbdd90277b18.png", VideoScope.PUBLIC, userRepo.findByNickname("user1").get(), musicRepo.findById(32l).get());
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user1").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user1").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(32l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(32l).get().getTitle(),TagType.TITLE));
            for(int j=0;j<i;j++){
                video.addHit();
            }
            video.addLikeWeight();
        ...
        for(int i=5;i<10;i++){
            Video video = Video.createVideo("/temp/c2d6f99a-8e0c-4413-a8ab-6e809b6b7d1b.webm",  "/temp/9e7de94f-a832-4af3-9125-fbdd90277b18.png", VideoScope.PUBLIC,userRepo.findByNickname("user1").get(), musicRepo.findById(28l).get());
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user1").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user1").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(28l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(28l).get().getTitle(),TagType.TITLE));
            for(int j=4;j<i;j++){
                video.addHit();
            }
            video.addLikeWeight();
            ...
            videos.add(video);
            em.persist(video);
            em.persist(Like.createLike(video,userRepo.findByNickname("user1").get()));
            em.persist(Like.createLike(video,userRepo.findByNickname("user2").get()));
        }
...
//        em.persist(Like.createLike(video1,user2));
        for(int i=10;i<15;i++){
            Video video = Video.createVideo("/temp/c2d6f99a-8e0c-4413-a8ab-6e809b6b7d1b.webm",  "/temp/9e7de94f-a832-4af3-9125-fbdd90277b18.png",VideoScope.PUBLIC,  userRepo.findByNickname("user2").get(), musicRepo.findById(28l).get());
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user2").get().getNickname(),TagType.NICKNAME));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(userRepo.findByNickname("user2").get().getNation().getName(),TagType.NATION));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(28l).get().getArtist().getName(),TagType.ARTIST));
            VideoTag.createVideoTag(video,tagRepo.findTestByNameAndType(musicRepo.findById(28l).get().getTitle(),TagType.TITLE));
            for(int j=7;j<i;j++){
                video.addHit();
        ...
        //clear
        em.persist(Clear.createClear(musicRepo.findById(32l).get(),userRepo.findByNickname("user1").get()));
//        em.persist(Clear.createClear(music1,user2));
        em.persist(Clear.createClear(musicRepo.findById(28l).get(),userRepo.findByNickname("user1").get()));
        em.persist(Clear.createClear(musicRepo.findById(28l).get(),userRepo.findByNickname("user2").get()));

        //follow
        em.persist(Follow.createFollow(userRepo.findByNickname("user1").get(),userRepo.findByNickname("user2").get()));
        em.persist(Follow.createFollow(userRepo.findByNickname("user2").get(),userRepo.findByNickname("user1").get()));
    }
