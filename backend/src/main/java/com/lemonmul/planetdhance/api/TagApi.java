...
public class TagApi {
    ...
    public ResponseEntity<?> userInfoAndUserVideos(@PathVariable Long tag_id,@PathVariable Long user_id){
        int page=0;
    ...
    public ResponseEntity<?> userVideos(@PathVariable Long tag_id,@PathVariable Long user_id,@PathVariable int page){
        try {
            Tag tag = tagService.findTagById(tag_id, page);
            User user = userService.findByNickname(tag.getName());
            Slice<Video> videoList;
