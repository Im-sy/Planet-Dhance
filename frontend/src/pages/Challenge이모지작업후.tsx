...
const startScoreTimer = function (duration : number) {
  ...
  console.log(end)
  setTimeout( ()=>{ setEmojis(prevState=>{ return [ ...prevState,  <Emoji emoji='💯'/>] }) },  end)
        ...
        if (nextNote != null) {   // 채점할 것이 있다면,
          if (
            ...
            countup < nextNote.delay + nextNote.duration
              ...
              //   max = 0.0;
              // }
                showEffect(nextNote, curMotionNum, max);
                song.next++;
                nextNote = song.notes[song.next];
                max = 0.0;
...
function showEffect(nextNote : number, curMotionNum : number ,rate : number) {
  //rate(perfect:1.0~0.9,good:0.9~0.5,miss:0.5~0.0)에 따라 이모티콘 피드백 표시
  //한 판정 내에 여러개 이모티콘 있고 랜덤으로 표시해주면 좋겠다....
  //프론트 분들 부탁합니다....ㅎㅎㅎ
  console.log(nextNote, curMotionNum)
  
  // 현재 판단해야 하는 경우에만 진행
  if (nextNote===curMotionNum){
    curMotionNum += 1
